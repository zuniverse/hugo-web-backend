...
from .utils import (
    add_to_erazed_files_list,
    allowed_file,
    escape_special_characters_within_string_inputs,
    get_current_time,
    get_file_header_and_body,
    list_all_files,
    sanitize_string,
)
...
app.config["MAX_CONTENT_LENGTH"] = app.config[
    "MAX_SIZE_UPLOAD_MEGS"
...
def receive_edit():
    ...
    if data["is_new_file"] == "False":
        ...
        """category is based on the fact that in Hugo archetype filename
...
@app.route("/deploy")
def send_to_prod():
    """Launch script to git pull / push and rsync to prod, and display
    result to template.
    """
    ...
    # debug:
    # print('app.config[ABS_PATH_DEPLOY_SCRIPT]='
    ...
    rc = subprocess.run(
        ...
        # capture_output: la sortie et l'erreur standard (stdout et stderr)
        # sont capturées
        capture_output=True,
        shell=True,
        text=True,
    ...
    return render_template(
        ...
        utc_dt=get_current_time(),
        rc=rc,
        # splitlines() because stdout comes out as a byte str with no eol
        stdout_as_list_of_lines=rc.stdout.splitlines(),
    )


@app.route("/import-updates")
def import_updates():
    """Launch script to git pull / push but NOT rsync to prod. Display
    result to template.
    """
    ...
    # print('app.config[ABS_PATH_IMPORT_UPDATES_NO_DEPLOY_SCRIPT]='
    #       + app.config['ABS_PATH_IMPORT_UPDATES_NO_DEPLOY_SCRIPT'])

    rc = subprocess.run(
        [
            app.config[
                "ABS_PATH_IMPORT_UPDATES_NO_DEPLOY_SCRIPT"
...
def delete_file():
    ...
    if request.method == "POST":
        # The requests.Response() Object contains the server's response
        # to the HTTP request
        # Get POST data.
        data = request.form
