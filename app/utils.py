...
def list_all_files(content_path, is_new_file=False):
    ...
    for root, dirs, files in os.walk(content_path):
        for file_name in files:
            ...
            img_path_relative_to_img_dir = re.search(
                r"([^/]+)/([^/]+)$", file_path, re.DOTALL
            ).group(0)
            params = {
                "q": file_path,
...
def sanitize_string(original_string):
    ...
    Then automatic just to make sure nothing was forgotten.
    @TODO use https://pypi.org/project/Unidecode/
    """
    ...
    clean_string = clean_string.replace('"', "-")
    clean_string = clean_string.replace("'", "-")
    # clean_string = clean_string.replace(".", '-') allows for file types
    # and dots are not invalid in file names
    clean_string = clean_string.replace("&", "-")
    clean_string = clean_string.replace("$", "-")
    clean_string = clean_string.replace("@", "-")
    clean_string = clean_string.replace("#", "-")
...
def parse_header_content(header_str):
    ...
    previous_line_was_a_comment = (
        False  # so as to not separate each line in case of multi line comments
    )
...
def get_current_time():
    ...
    return datetime.datetime.utcnow().isoformat(
        timespec="seconds"
    )  # Daylight savings time Not taken into account
...
def add_to_erazed_files_list(file_path):
    ...
    try:
        erazed_files = open(
            app.config["LOCATION_OF_STATIC_DIR"] + app.config["LIST_OF_ERAZED_FILES"],
            "a",
        )
